...
class BasePillarTest(ModuleCase):
    ...
    def test_minion_cache_should_cache_files(self, tempdir):
        ...
        with patch('salt.loader.grains', return_value={}), \
                patch('salt.minion.SMinion.gen_modules'), \
                patch('salt.minion.SMinion.eval_master'), \
                patch('salt.minion.install_zmq'), \
                patch('salt.minion.ZMQDefaultLoop.current'):
